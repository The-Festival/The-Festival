...
<html lang="en">
  ...
  <body>
    <h1 class = "d-flex justify-content-center">Add jazz ticket to order : <?php echo $order_id ?></h1>
        ...
        <div class = "row">
            <div class = "col-12">
                <form action = "/admin/ticketdashboard" method = "post">
                    <input type="hidden" name  = "addJazzTicketToOrder">
                    <input type="hidden" name="order_id" value="<?php echo $order_id ?>">
                    <div class = "form-group">
                        <label for = "event_id">Jazz event :</label>
                    <select name="event_id" class = "form-select" Required>
                        <?php foreach($jazzEvents as $event){?>
                            <option value="<?php echo $event['event_id']?>">ID : <?php echo $event['event_id']?> Naam: <?php echo $event['name']?></option>
                            <?
                            }
                            ?>
                    </select>
                    ...
                    <div class = "form-group">
                        ...
                        <button type = "submit" class = "btn btn-primary mt-2">Add Jazz Ticket</button>
                    </div>
