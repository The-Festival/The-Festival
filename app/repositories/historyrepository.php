...
class HistoryRepository extends Repository{
    ...
    public function getLocationByPOIID($id){
        try {
            $stmt = $this->connection->prepare("SELECT location_id, detail_id, streetname, postalcode, city, housenumber FROM `Location` WHERE `detail_id` = :id;");
            $stmt->bindParam(':id', $id);
            $stmt->execute();
    ...
    public function getLocations(){
        try {
            $stmt = $this->connection->prepare("SELECT DISTINCT location_id, detail_id, streetname, postalcode, city, housenumber FROM `Location`;");
            $stmt->execute();
            $stmt->setFetchMode(PDO::FETCH_CLASS, 'Location');
            $result = $stmt->fetchAll();
